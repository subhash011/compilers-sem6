...
struct
    ...
    and strs_dec (VarDec v) = let
        ...
        | strs_dec (TypeDec t) = let val {name, type_} = t in ["TypeDec(name = ", sym_name name, ", type_ = "] @ strs_type type_ @ [")"] end
    ...
    and strs_ast (Decs decs) = (["Decs(["] @ strs_decs decs @ ["])"] @ ["\n"])
        | strs_ast (Expr exp) = (["Expr("] @ strs_exp exp @ [")"] @ ["\n"])
